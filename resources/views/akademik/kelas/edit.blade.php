		...
		<div class="col-lg-12">
			...
			<div class="panel panel-inverse" data-sortable-id="form-plugins-3">
				...
				<div class="panel-heading">
					...
					<h4 class="panel-title">FORM EDIT KELAS </h4>
				</div>
				<!-- end panel-heading -->
				<!-- begin alert -->
				...
				<form class="form-horizontal form-bordered" method="POST" action="/perkuliahan/prodi/{{$id_prodi}}/kelas/{{$data['id_kelas_kuliah']}}/update">
                        @csrf
                        <div class="form-group row">
                            <label class="col-md-4 col-form-label">Nama Prodi</label>
                            <div class="col-md-8">
                                <input type="text" class="form-control" readonly value="{{$data['nama_program_studi']}}"/>
                            </div>
                        </div>
                        <div class="form-group row">
                            <label class="col-md-4 col-form-label">Semester</label>
                            <div class="col-md-8">
                                <input type="text" class="form-control" readonly value="{{$data['nama_semester']}}"/>
                            </div>
                        </div>
						<div class="form-group row">
							...
							<div class="col-md-8">
								<select class="default-select2 form-control" name="id_matkul" required>
									@foreach ($matkul as $item)
									  	@if($data['id_matkul'] == $item['id_matkul'])
										<option value="{{$item['id_matkul']}}" selected>{{$item['kode_mata_kuliah']}} - {{$item['nama_mata_kuliah']}}</option>
										@else
						...
						<div class="form-group row">
							...
							<div class="col-md-8">
							<input class="form-control" type="text" name="nama_kelas" value="{{$data['nama_kelas_kuliah']}}" required/>
							</div>
                        </div>
						<div class="form-group row">
